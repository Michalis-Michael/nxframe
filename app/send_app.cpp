               ...
               std::atomic<bool>& shutdownRequested)
{
    ...
    // Optional machine-local CPU frequency cap. The guard restores the previous
    // cpufreq state automatically when runSendApp() returns after Ctrl+C,
    // normal stop, or initialization failure.
    nxframe::CpuProfileGuard cpuProfileGuard;
    if (!cpuProfileName.empty()) {
        std::string cpuError;
        if (!cpuProfileGuard.applyProfile(cpuProfileName, cpuProfileConfigPath, &cpuError)) {
            std::cerr << "[CPU] Error: " << cpuError << "\n";
            return -1;
        }
    }
