...
// Send one MPEG-TS payload through the active SRT socket. The caller already
// controls packet boundaries, so SRT message API mode is used for live chunks.
bool SRTStreamer::sendPacket(const unsigned char* data, int size)
{
    ...
    const int payload = (config_snapshot.payload_size > 0) ? config_snapshot.payload_size : kDefaultPayloadSize;
    ...
    while (offset < size) {
        ...
        const int chunk = std::min(payload, size - offset);
        int sent = SRT_ERROR;
        ...
        if (sent <= 0) {
            app_send_failures_.fetch_add(1, std::memory_order_relaxed);
            setLastError("send returned non-positive byte count");
            std::cerr << "[SRT] send returned " << sent << ", aborting packet send.\n";
            setState(ConnectionState::Failed);
            return false;
        }
