...
void validateMpegTs(const json& root, PresetValidator::Result& r)
{
    ...
    for (const char* key : {"muxrate", "mux_rate", "ts", "ts_bitrate", "transport_rate", "ts_rate"}) {
        if (!mpegts->contains(key)) {
            continue;
        }
        const json& v = (*mpegts)[key];
        if (!v.is_number_integer() && !v.is_number_float() && !v.is_string()) {
            r.errors.push_back(pathJoin(p, key) + " must be a bitrate integer in bps or string like '56Mbps'");
            continue;
        }
        if (v.is_number_integer() && v.get<int64_t>() > 0 && v.get<int64_t>() < 1000000) {
            r.warnings.push_back(pathJoin(p, key) + " is below 1 Mbps; use 56000000 for 56 Mbps");
        }
    }
    (void)p;
}
